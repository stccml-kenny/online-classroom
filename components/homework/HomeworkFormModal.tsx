...
import {
  X, Save, BookOpen, UploadCloud, FileText, Check, ChevronDown, Music, Video, Loader2, ExternalLink, Globe, Plus, Calendar, MapPin, GraduationCap, Lock
} from 'lucide-react';
import { HomeworkItem, HomeworkAttachment, extractYoutubeId, getGoogleLinkMeta, YoutubeIcon } from './HomeworkCard';
import { CourseItem, getCourseDisplayName } from './HomeworkSetupModal';
...
export const HomeworkFormModal: React.FC<HomeworkFormModalProps> = ({
  ...
  useEffect(() => {
    if (initialData) {
      setBranch(initialData.branch || defaultBranch || branches[0] || '');
      ...
      // ⭐ 需求：如果已鎖定課程則帶入該課程；否則新增家課不要預選課程 (留空讓老師選擇)
  ...
  }, [initialData, isOpen, branches, courses, courseItems, defaultBranch, defaultCourse, isLocked, targetUnitId, targetUnitTitle]);
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-3.5 overflow-y-auto text-sm">
          ...
          {isLocked ? (
            <div className="p-3 bg-orange-50/70 border border-orange-200/90 rounded-xl space-y-2">
              <div className="flex items-center justify-between text-xs text-orange-900 font-bold">
                <span className="flex items-center gap-1.5">
                  <Lock size={13} className="text-[#FF6B57]" />
                  <span>已鎖定指定學校與課程</span>
                </span>
                <span className="text-[10px] bg-[#FF6B57] text-white px-2 py-0.5 rounded-full font-bold shadow-2xs">
                  專屬此課程
                </span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="bg-white p-2.5 rounded-lg border border-orange-100 shadow-2xs">
                  <span className="text-[10px] text-gray-400 block mb-0.5">學校 / 分校</span>
                  <span className="font-bold text-gray-800">{branch || '全部分校'}</span>
                </div>
                <div className="bg-white p-2.5 rounded-lg border border-orange-100 shadow-2xs min-w-0">
                  <span className="text-[10px] text-gray-400 block mb-0.5">所屬課程</span>
                  <span className="font-bold text-[#FF6B57] truncate block">{courseName}</span>
                </div>
              </div>
            </div>
          ) : (
              ...
              <div>
                ...
                {branches.length > 0 ? (
                  <select
                    ...
                    className="w-full p-2.5 border border-purple-200 rounded-lg bg-purple-50/50 text-xs font-medium text-gray-800 outline-none focus:bg-white focus:border-[#FF6B57]"
                  >
                    {branches.length > 1 && (
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  <label className="block text-xs font-semibold text-gray-600 flex items-center gap-1">
                    <GraduationCap size={12} className="text-[#FF6B57]" />
                    <span>課程 (Course)</span> <span className="text-[#FF6B57]">*</span>
                  </label>
                  {branch && branch !== '全部分校' ? (
                ...
                {availableCourses.length > 0 ? (
                  <select
                    ...
                    className="w-full p-2.5 border border-orange-200 rounded-lg bg-orange-50/40 text-xs font-medium text-gray-800 outline-none focus:bg-white focus:border-[#FF6B57]"
                    required
                  >
